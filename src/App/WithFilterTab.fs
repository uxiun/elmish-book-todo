...
type Filter = { Completed: bool option }

let defaultFilter: Filter = { Completed = None }
...
let tabProps (completed: bool option) (state: State) (dispatch: Msg -> unit) =
  (match completed with
   | None -> [ prop.children [ Html.a [ prop.text "All" ] ] ]
   | Some true -> [ prop.children [ Html.a [ prop.text "Completed" ] ] ]
   | Some false -> [ prop.children [ Html.a [ prop.text "Not Completed" ] ] ])
  @ [
    if completed = state.Filter.Completed then
      prop.className "is-active"
    prop.onClick (fun _ -> dispatch (SetFilter { state.Filter with Completed = completed }))
  ]

let renderFilterTab (state: State) (dispatch: Msg -> unit) : ReactElement =
  div
    [
      "tabs"
      "is-toggle"
    ...
    ([
     ...
     ./ List.map (fun o -> tabProps o state dispatch ./ Html.li))
  ...
  Html.div [
    ...
    ([ appTitle ]
     @ [
       ...
       ./ List.map (fun f -> f state dispatch))
    ./ prop.children
  ]
